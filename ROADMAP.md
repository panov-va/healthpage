...
- [ ] **1.7** Публичный `GET /pages/{slug}/summary` и `GET /pages/{slug}/components`.
- [ ] **1.8** `public-ssr`: публичная страница статуса (SSR) — общий статус + дерево компонентов;
      работает независимо от админки.
- [ ] **1.9** `public-ssr`: базовый лендинг (SSR) для SEO — главная, тарифы (заглушки).
- [ ] **1.10** Админка (React+FSD): экран входа, список страниц, CRUD компонентов/групп, ручная
