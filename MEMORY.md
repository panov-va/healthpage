...
**Фаза:** Этап 1 (Ядро домена). Этап 0 закоммичен (+ возможно 1.1). Задачи 1.1–1.4 написаны
и проверены, ждут коммита человеком.
**Следующий шаг:** Этап 1.5 — API CRUD страниц/групп/компонентов (вкл. подкомпоненты) и ручная
смена current_status, строго по openapi.yaml. Затем 1.6 (логика общего статуса в API),
1.7 (публичные summary/components).
