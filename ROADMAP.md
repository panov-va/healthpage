...
- [ ] **4.2.1** Приватные страницы по **списку email** (отложено из 4.2, решение человека): таблица
      `allowed_emails`, запрос magic-link на почту (через worker-email), верификация → токен доступа
      (тот же `X-Page-Access`-механизм). Требует расширения контракта/модели.
- [~] **4.3** Собственный домен: верификация CNAME + автонастройка TLS (ACME).
  ...
  - [ ] **4.3.2** ACME-сервис `cmd/tls-manager` (lego): выпуск/хранение/продление сертов для
        verified-доменов (HTTP-01; таблица сертификатов; renewal-loop).
  - [ ] **4.3.3** Edge-прокси: TLS-терминация по SNI (серты из 4.3.2), HTTP-01 passthrough на :80,
        роутинг по Host на public-ssr (резолв домен→страница). Acceptance 4.3 закрывается здесь.
