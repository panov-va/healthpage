...
**Следующий шаг:** Этап 3.3 — движок уведомлений: публикация событий (новый инцидент, обновление с
notify, старт/конец работ) в exchange `notifications`; создание записей `Notification` (идемпотентность
по `Notification.id`); ретраи с backoff через `delayed.events`; учёт `attempts`. Топология (3.2) и
публикатор `queue.Publisher` уже готовы. Затем worker'ы (3.4 email, 3.7 telegram, …).
