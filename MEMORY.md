...
**Фаза:** Этап 1 (Ядро домена). Этап 0 закоммичен (+ возможно 1.1). Задачи 1.1–1.8 написаны
и проверены, ждут коммита человеком.
**Следующий шаг:** Этап 1.9 — `public-ssr` (Next.js): базовый лендинг (SSR) для SEO — главная,
тарифы (заглушки). Затем 1.10 (админка React). Это закрывает Этап 1 (acceptance).
